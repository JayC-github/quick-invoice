...
import LineItemEditor, { type LineItemRow } from '../components/LineItemEditor';
import type { Invoice, Client } from '../types/api';

let nextEditId = 1000;
function toEditRow(item: { description: string; quantity: number; unitPrice: number }): LineItemRow {
  return {
    id: `edit-${nextEditId++}`,
    description: item.description,
    quantity: String(item.quantity),
    unitPrice: String(fromCents(item.unitPrice)),
  };
}
...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [inv, clientData] = await Promise.all([
        ...
        setTaxRateInput(String(inv.taxRate * 100));
        setNotes(inv.notes || '');
        setLineItems(inv.lineItems.map(toEditRow));
      } catch (err) {
        if (!cancelled) {
      ...
      <div style={cardStyle}>
        <form onSubmit={handleSubmit} style={formStyle}>
          ...
          <div style={buttonRowStyle}>
            <button type="button" onClick={() => navigate(`/invoices/${id}`)} style={cancelButtonStyle} disabled={submitting}>
              Cancel
            </button>
